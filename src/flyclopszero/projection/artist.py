...
from flyclopszero.projection.renderer import SceneRenderer # Import our new headless renderer
...
class Artist:
    ...
    """
    def __init__(self, projector_config: Dict, camera_config: Dict, calibration: Calibration):
        import pygame
        self.pygame = pygame
        ...
        self.pygame.init()
        self.screen = self.pygame.display.set_mode((self.projector_width, self.projector_height), self.pygame.NOFRAME)
    ...
    def render(self, instructions: List[Dict]):
        ...
        pygame_surface = self.pygame.surfarray.make_surface(projector_image_rgb.swapaxes(0, 1))
    ...
    def transform_camera_to_projector_map(self, camera_image: np.ndarray) -> np.ndarray:
        ...
        projection_space_image = cv2.remap(camera_image, map1x, map1y, interpolation=cv2.INTER_LINEAR)
        
        correction_method = self.calibration.get('projector_correction_method', 'distortion')
        ...
        corrected_projector_image = cv2.remap(projection_space_image, map2x, map2y, interpolation=cv2.INTER_LINEAR)
